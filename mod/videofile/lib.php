...
function videofile_pluginfile($course,
    ...
    if ($filearea !== 'videos' && $filearea !== 'captions' && $filearea !== 'posters' && $filearea !== 'video_converter') {
...
/**
 * Return The total progress of the one video
 * @param  cmid
 * @return percentage of progress of the one video for the user
 */
function video_status($cmid, $userid){
    global $DB;
    $total_progress = $DB->get_records_sql('SELECT * FROM {video_attempts} 
                                            WHERE cmid = ? AND userid = ? 
                                            ORDER BY percentage 
                                            DESC', array($cmid, $userid), 0, 1);
    if(count($total_progress) > 0){
        foreach ($total_progress as $video_progress) {
            return $video_progress->percentage;
        }
    } else {
        return 0;
    }
...
function videofile_get_completion_state($course, $cm, $userid, $type) {
    global $CFG, $DB;

    $result = $type; // Default return value

    // Get glossary details
    if (!($video = $DB->get_record('videofile',array('id'=>$cm->instance)))) {
        throw new Exception("Can't find glossary {$cm->instance}");
    }

    $records = $DB->get_records_sql('SELECT * FROM {video_attempts} 
                                    WHERE userid = ? AND percentage >= ?', array($userid, $video->videoprogress));
    if(count($records) > 0){
        $value = true;
    } else {
        $value = false;
