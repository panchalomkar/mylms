...
class videofile {
    ...
    public function add_instance(stdClass $formdata) {
        ...
        $add->video_type    = 0;
        if(!$formdata->videoprogress || $formdata->videoprogressenabled == 0){
            $add->videoprogress = 0;
        } else{
            $add->videoprogress = $formdata->videoprogress;
        }
        /**
         * Now the video_enabled input is validated when add too, and the php version,
         * makes the validation deprecated, we change this to a string validation.
         * @author Hugo S.
         * 07-06-2018
         * @paradiso
         * @ticket 8
         */
        if($formdata->video_enabled == "1"){
            $add->video_url = '';
        } else {
            $add->video_url = $formdata->video_url;
        }

        $add->forward       = $formdata->forward;
    ...
    public function update_instance($formdata) {
        ...
        /**
        * If video url is not enable, set the value as empthy string
        * @author ♦ Andrés Ag. ♦
        ...
        */
        if($formdata->video_enabled == "1"){
            ...
            $update->video_url = $formdata->video_url;
        }

        if(!$formdata->video_type){
            $update->video_type = 0;
        } else {
            $update->video_type = $formdata->video_type;
        }
        
        // 878 Ticket If exist videoprogressenabled then codition is true
        if (isset($formdata->videoprogressenabled)) {
            if (!$formdata->videoprogress || $formdata->videoprogressenabled == 0) {
                $update->videoprogress = 0;
            } else {
                $update->videoprogress = $formdata->videoprogress;
            }
        }
        $update->forward = $formdata->forward;
