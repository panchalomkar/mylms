...
$id      = required_param('id', PARAM_INT); // ID in ilt_notice.
...
$notice = null;
if ($id > 0) {
    $notice = $DB->get_record('ilt_notice', array('id' => $id));
}

$PAGE->set_url('/mod/ilt/sitenotice.php', array('id' => $id, 'd' => $d, 'confirm' => $confirm));
...
$title = get_string('addnewnotice', 'ilt');
if ($notice != null) {
    $title = $notice->name;
}
...
if (!empty($d)) {
    ...
    if (!$confirm) {
        ...
        echo $OUTPUT->confirm(get_string('noticedeleteconfirm', 'ilt', $info),
        ...
        $DB->delete_records('ilt_notice', array('id' => $id));
        $DB->delete_records('ilt_notice_data', array('noticeid' => $id));
...
$mform = new mod_ilt_sitenotice_form(null, compact('id', 'customfields'));
...
if ($fromform = $mform->get_data()) { // Form submitted.
    ...
    if ($notice != null) {
        $todb->id = $notice->id;
        $DB->update_record('ilt_notice', $todb);
    } else {
        $notice = new stdClass();
        $notice->id = $DB->insert_record('ilt_notice', $todb);
    }
    ...
    foreach ($customfields as $field) {
        ...
        ilt_save_customfield_value($field->id, $fromform->$fieldname, $notice->id, 'notice');
    }
    $transaction->allow_commit();
    redirect($returnurl);
    ...
    foreach ($customfields as $field) {
        $fieldname = "custom_$field->shortname";
        $toform->$fieldname = ilt_get_customfield_value($field, $notice->id, 'notice');
    }
