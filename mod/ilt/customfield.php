...
$id      = required_param('id', PARAM_INT); // ID in ilt_session_field.
...
$field = null;
if ($id > 0) {
    if (!$field = $DB->get_record('ilt_session_field', array('id' => $id))) {
        print_error('error:fieldidincorrect', 'ilt', '', $id);
    }
}

$PAGE->set_url('/mod/ilt/customfield.php', array('id' => $id, 'd' => $d, 'confirm' => $confirm));
...
$returnurl = "$CFG->wwwroot/admin/settings.php?section=modsettingilt";

// Header.

$title = get_string('addnewfield', 'ilt');
if ($field != null) {
    $title = $field->name;
}
...
if (!empty($d)) {
    ...
    if (!$confirm) {
        ...
        echo $OUTPUT->confirm(get_string('fielddeleteconfirm', 'ilt', format_string($field->name)),
        ...
        try {
            if (!$DB->delete_records('ilt_session_field', array('id' => $id))) {
                throw new Exception(get_string('error:couldnotdeletefield', 'ilt'));
            }

            if (!$DB->delete_records('ilt_session_data', array('fieldid' => $id))) {
                throw new Exception(get_string('error:couldnotdeletefield', 'ilt'));
            }
...
$mform = new mod_ilt_customfield_form(null, compact('id'));
...
if ($fromform = $mform->get_data()) { // Form submitted.
    ...
    $todb->possiblevalues = implode(ILT_CUSTOMFIELD_DELIMITER, $posvals);
    ...
    if ($field != null) {
        $todb->id = $field->id;
        if (!$DB->update_record('ilt_session_field', $todb)) {
            print_error('error:couldnotupdatefield', 'ilt', $returnurl);
        }
    } else {
        if (!$DB->insert_record('ilt_session_field', $todb)) {
            print_error('error:couldnotaddfield', 'ilt', $returnurl);
        }
    }
    ...
    $valuearray = explode(ILT_CUSTOMFIELD_DELIMITER, $field->possiblevalues);
