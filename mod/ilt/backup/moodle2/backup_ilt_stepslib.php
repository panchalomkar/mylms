...
class backup_ilt_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $ilt = new backup_nested_element('ilt', array('id'), array(
        ...
        $session = new backup_nested_element('session', array('id'), array(
            'ilt', 'capacity', 'allowoverbook', 'details', 'datetimeknown', 'duration', 'normalcost',
        ...
        // Define sources.
        $ilt->set_source_table('ilt', array('id' => backup::VAR_ACTIVITYID));

        $session->set_source_table('ilt_sessions', array('ilt' => backup::VAR_PARENTID));

        $sessionsdate->set_source_table('ilt_sessions_dates', array('sessionid' => backup::VAR_PARENTID));

        if ($userinfo) {
            $signup->set_source_table('ilt_signups', array('sessionid' => backup::VAR_PARENTID));

            $signupstatus->set_source_table('ilt_signups_status', array('signupid' => backup::VAR_PARENTID));

            $sessionrole->set_source_table('ilt_session_roles', array('sessionid' => backup::VAR_PARENTID));

            $sessiondataelement->set_source_table('ilt_session_data', array('sessionid' => backup::VAR_PARENTID));
        }
        ...
        $sessiondataelement->annotate_ids('ilt_session_field', 'fieldid');

        // Define file annotations.
        // None for ILT.

        // Return the root element (ilt), wrapped into standard activity structure.
        return $this->prepare_activity_structure($ilt);
    }
}
