...
class mod_pdfjsfolder_renderer extends plugin_renderer_base {
    ...
    public function pdf_header($pdfjsfolder, cm_info $cm) {
        ...
        $output .= $this->output->heading($name, 2);
    ...
    protected function htmlize_folder($tree,
        ...
        foreach ($dir['files'] as $pdf) {
            ...
            $fileurlforcedownload = moodle_url::make_pluginfile_url(
                ...
                $filename,
                true);
            $main_root = $CFG->wwwroot;
            $fpath = '/'.$pdf->get_component().'/'.$pdf->get_filearea().'/'.$pdf->get_itemid().'/'.$pdf->get_filepath();
            $moodle_url = $main_root.'/pluginfile.php&id='.$pdf->get_contextid().'&path='.$fpath.'&name='.$filename;
            ...
            if (file_extension_in_typegroup($filename, 'web_image')) {
                ...
                $url = $pdfjsfolderurl . '?files=' . $moodle_url;
                $isimage = false;
            }
