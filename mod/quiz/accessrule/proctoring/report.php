...
if (
    has_capability('quizaccess/proctoring:deletecamshots', $context, $USER->id)
    && $studentid != null
    && $cmid != null
    && $courseid != null
    && $reportid != null
...
if (
    ...
    foreach ($sqlexecuted as $info) {
        ...
        $btn = '<a onclick="' . $con . '" href="?courseid=' . $courseid .
            ...
            $zipurl = new moodle_url('/mod/quiz/accessrule/proctoring/download_images_zip.php', [
                'courseid' => $courseid,
                'quizid' => $cmid,
                'studentid' => $info->studentid,
                'reportid' => $info->reportid
            ]);
            $zipicon = '<a href="' . $zipurl . '" title="Download all images as ZIP"><i class="icon fa fa-file-archive-o fa-fw"></i></a>';
    
            $data[] = '<a href="?courseid=' . $courseid . '&quizid=' . $cmid . '&cmid=' . $cmid . '&studentid=' . $info->studentid . '&reportid=' . $info->reportid . '"><i class="icon fa fa-folder-o fa-fw "></i></a>'
                     . $zipicon . $btn;
    ...
    if ($studentid != null && $cmid != null && $courseid != null && $reportid != null) {
        ...
        $userinfo = '
<div style="width: 250px; height: auto; border: 1px solid #ccc; border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); text-align: center; padding: 16px; font-family: Arial, sans-serif; background: #fff;">
    <div class="accessruleimageuser" style="margin-bottom: 10px;">
        ' . $OUTPUT->user_picture($user, ['size' => 100, 'class' => 'profile-pic']) . '
    </div>
    ...
    <a href="' . $analyzeurl . '" style="display:none; background-color: #003152; color: #fff; padding: 8px 14px; border-radius: 6px; text-decoration: none; font-size: 14px;">
        Analyze Images
    </a>
</div>';
