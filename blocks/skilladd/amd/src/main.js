...
export const init = (blockid) => {
    ...
    let addButton = document.querySelector('div[data-instanceid="' + instanceid + '"] .block-skilladd-input-button');
    addButton.addEventListener('click', addItem);

    let taskInput = document.querySelector('div[data-instanceid="' + instanceid + '"] #block-skilladd-input-' + instanceid);
    ...
    let list = document.querySelector('div[data-instanceid="' + instanceid + '"] ul.block-skilladd-list');
...
const addItem = (clickEvent) => {
    let instanceid = clickEvent.target.closest('.block-skilladd').dataset.instanceid;
    let name = document.querySelector('div[data-instanceid="' + instanceid + '"] input.block-skilladd-input');
    let list = document.querySelector('div[data-instanceid="' + instanceid + '"] ul.block-skilladd-list');
    ...
    Ajax.call([{
        methodname: 'block_skilladd_add_item',
        args: {
...
const fetchList = async (instanceid) => {
    return Ajax.call([{
        methodname: 'block_skilladd_get_items',
        args: {
            instanceid: instanceid,
        },
    }])[0]
...
const addItems = items => {
    ...
    const pendingPromise = new Pending('blocks/skilladd:addItems');
...
const renderItem = (itemData) => {
    Templates.render('block_skilladd/task_item', itemData).done(function(html) {
        let list = document.querySelector('div[data-instanceid="' + itemData.instanceid + '"] ul.block-skilladd-list');
...
const addItemEditEvent = (item) => {
    let itemname = item.querySelector('label.block-skilladd-itemname');
    let input = item.querySelector('input.block-skilladd-item-edit-input');
    item.querySelector('button.block-skilladd-item-edit').addEventListener('click', () => {
    ...
    item.querySelector('input.block-skilladd-item-edit-input').addEventListener('keydown', (e) => {
        if (e.code === 'Enter') {
            updateItemName(item, itemname, input);
        }
    });
...
const updateItemName = (item, itemname, input) => {
    ...
    let itemData = {
        ...
        name: item.querySelector('label.block-skilladd-itemname').innerText,
        position: parseInt(item.dataset.position),
        complete: item.classList.contains('block-skilladd-item-complete')
    };
...
const addCompleteItemEvents = (item) => {
    item.querySelector('button.block-skilladd-item-complete').addEventListener('click', () => {
        let itemData = {
            id: parseInt(item.dataset.itemid),
            name: item.querySelector('label.block-skilladd-itemname').innerText,
            position: parseInt(item.dataset.position),
        };
        if (item.classList.contains('block-skilladd-item-complete')) {
            item.classList.remove('block-skilladd-item-complete');
            itemData.complete = false;
        } else {
            item.classList.add('block-skilladd-item-complete');
...
const addDeleteEvent = (item) => {
    item.querySelector('.block-skilladd-item-delete').addEventListener('click', () => {
        item.remove();
...
const updateItems = (instanceid, items) => {
    Ajax.call([{
        methodname: 'block_skilladd_update_items',
        args: {
            instanceid: instanceid,
            items: items
        },
...
const deleteItem = (instanceid, itemid) => {
    Ajax.call([{
        methodname: 'block_skilladd_delete_item',
        args: {
            instanceid: instanceid,
            itemid: itemid
        },
