...
namespace block_skilladd\privacy;
...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $collection->add_database_table('block_skilladd_items', [
    ...
    public static function get_users_in_context(userlist $userlist) {
        foreach ($userlist->get_userids() as $userid) {
            $sql = "SELECT userid FROM {block_skilladd_items} WHERE userid = ?";
            $params = [$userid];
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        foreach ($userlist->get_userids() as $userid) {
            $DB->delete_records('block_skilladd_items', ['userid' => $userid]);
        }
    }
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        $items = [];
        $results = $DB->get_records('block_skilladd_items', ['userid' => $contextlist->get_user()->id]);
        ...
        if (!empty($items)) {
            ...
            \core_privacy\local\request\writer::with_context($contextlist->current())->export_data([
                get_string('pluginname', 'block_skilladd')], $data);
        }
    }
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        if ($context instanceof \context_user) {
            $DB->delete_records('block_skilladd_items', ['instanceid' => $context->instanceid]);
        }
    }
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        global $DB;
        $DB->delete_records('block_skilladd_items', ['userid' => $contextlist->get_user()->id]);
    }
}
