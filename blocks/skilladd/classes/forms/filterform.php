...
class filter_form extends moodleform {
    ...
    public function definition() {
       // $userid = optional_param('userid', 0, PARAM_INT);                                                        
        $users = self::get_search_areas_list();                                                                                                                            
        $options = array(                                                                                                           
            'multiple' => false,                                                  
            'noselectionstring' => get_string('allusers', 'block_skilladd'),                                                                
        );    
        ...
        $mform->addElement('autocomplete', 'userid', get_string('selectusers','block_skilladd'), $users, $options);
        $mform->addRule('userid', get_string('requiredfield', 'block_skilladd'), 'required', null, 'client', false, false);
    ...
    public function get_search_areas_list(){
        ...
        if ($selectedcompany) {
            $query = "AND cu.companyid = $selectedcompany";
        }
        
        $urs = array();
        $urs[''] = get_string('allusers','block_skilladd');
        $users = $DB->get_records_sql("SELECT u.id, u.username FROM {user} u 
        INNER JOIN {company_users} cu ON u.id = cu.userid WHERE u.deleted = 0 AND u.suspended = 0 $query");
