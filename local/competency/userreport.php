...
$activepage = 'userreport';
$context = context_system::instance();
require_login();
$PAGE->set_context($context);
$PAGE->set_title(get_string('userreport', 'local_competency'));
$PAGE->set_url($CFG->wwwroot . '/local/competency/userreport.php');
$PAGE->set_heading(get_string('userreport', 'local_competency'));
...
$listCompetencyCount = getListLoginUserRptCount($query);
$pagesArr = getPaginationDisplay($listCompetencyCount, $selectPageNo, $limit);
$pages = $pagesArr[0];
$start = $pagesArr[1];
...
$searchSqlcomp = "select lr.id, concat(u.firstname,' ',u.lastname) AS fullname, u.id as userid, u.username,ct.title, cc.id as ccid, cc.name as subcompetency, c.id as comptencyid, c.comptencyname as subsubcomp,lr.rating,
					MAX(CASE WHEN r.shortname = 'sbuhead' THEN ud.data END) 'buhead', 
...
// enroll course button -<td> <a href="#" class="btn btn-primary" onclick="enrollcourse('.$competency_categorys_val->userid.','. $subsubcomptencyid .','. $subcomptencyid .')" > Enroll Course </a></td>
...
$viewcontentbody = '<div class="row" style="text-align:center;margin-bottom:10px;margin-top:10px;">
	...
	<div class="col-md-1"><a href="userreport.php" class="btn btn-primary">Clear</a></div>
...
if ($pages > 1) {
	...
	$pagination = viewuserreportpagination($selectPageNo, $pages, $userid, $ctid, $ccid, $competenciesid, $rateid);
}
echo $pagination;
?>
</form>
