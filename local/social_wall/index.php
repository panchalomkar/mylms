...
$PAGE->requires->css(new moodle_url("/local/social_wall/socialwall-style.css"));
$PAGE->requires->jquery();
// Page configurations.
$PAGE->set_url('/local/social_wall/home.php');
...
echo html_writer::start_tag('div', array('class' => "_social-wall-container $class"));
echo html_writer::start_tag('div', array('class' => '_custom-header-container'));
echo html_writer::start_tag('div', array('class' => '_custom-header-row'));
...
$pagebg = 'https://static.vecteezy.com/system/resources/previews/007/382/832/large_2x/education-illustration-of-community-economic-and-social-learning-to-increase-knowledge-in-careers-and-jobs-in-world-of-adult-work-landing-page-web-website-banner-ads-card-apps-brochure-flyer-free-vector.jpg';
//   $pagebg = $CFG->wwwroot.'/local/social_wall/slider.jpg';
$prescss = "";
if (isset($pagebg)) {
	$prescss = 'background-image: url("' . $pagebg . '"); background-size:cover; background-position:top center;height:400px;';
} else {
...
echo html_writer::start_tag('div', ['class' => 'col-sm-12']);
echo html_writer::start_tag('div', ['class' => 'd-flex _profile-component']);
echo html_writer::start_tag('div', ['class' => '_profile-image']);
echo html_writer::start_tag('a', ['href' => $CFG->wwwroot.'/user/edit.php?id='.$USER->id.'&returnto=profile#collapseElement-1','title'=>'Profile']);


echo html_writer::tag('img', "", array('src' => "$profileimageurl",'id' => '','value'=>$USER->id));
echo html_writer::end_tag('a');
echo html_writer::end_tag('div');
echo html_writer::start_tag('div', ['class' => '_profile-information']);
echo html_writer::tag('h2', $USER->firstname." ".$USER->lastname,['class'=>'mb-0 mt-3']);
echo html_writer::start_tag('div', ['class' => 'd-flex items-center text-muted']);
echo html_writer::tag('h3', get_string('noofpost', 'local_social_wall'));
echo html_writer::tag('h3', $count_post,['class'=>'_count_post']);
echo html_writer::end_tag('div');
echo html_writer::start_tag('div', ['class' => '_timeline_div','']);
...
echo html_writer::start_tag('div', array('class' => '_custom-header-row-2'));
echo html_writer::start_tag('div', ['class' => 'row']);
echo html_writer::start_tag('div', ['class' => 'notify-box col-md-4 col-xs-11 col-sm-11 sliding-sidebar','id'=>'notify-box-1']);
echo html_writer::start_tag('div', ['class' => '_cardone ', 'id' => 'updates-box','style'=>'']);
...
echo html_writer::start_tag('div', ['class' => 'col-12 px-0']);
