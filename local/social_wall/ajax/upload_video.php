<?php
/**
 * This file handles the video file upload operations.
 * 
...
$response = array( 'status' => false, 'message' => get_string('novideo', 'local_social_wall') );
$upload = 1;
$target_dir = $CFG->dirroot ."/local/social_wall/uploads/videos/";
$target_wwwroot = $CFG->wwwroot ."/local/social_wall/uploads/videos/";
for($i = 0; $i < count($_FILES["file"]["name"]); $i++){
    ...
    if( !in_array( $videoFileType, $extensions )) {
        $response['message'] = get_string('videoextensionallowed','local_social_wall');
        $response['status'] = false;
        $response['videoFileType'] = $videoFileType;
    ...
    if( $upload ){
        if ( 0 < $_FILES['file']['error'][$i] ) {
            $response['message'] =  get_string('probleminuploadingvideo','local_social_wall') . $_FILES['file']['error'][$i];
            $response['status'] = false;
        }else {
            ...
            if( $outputfile ){
                ...
                $videosurl = file_encode_url("$CFG->wwwroot/pluginfile.php",'/'. $context->id. '/local_social_wall/social_wall_files/0/'.$outputfile);
                $response['message'] = get_string('uploadsuccess','local_social_wall');
                $response['url'][$i] = $videosurl;
