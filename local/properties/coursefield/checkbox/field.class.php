 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_field_checkbox extends course_field_base {
     ...
     * @param int $courseid
     */
    public function course_field_checkbox($fieldid=0, $courseid=0) {
        global $DB;
        // First call parent constructor.
        $this->course_field_base($fieldid, $courseid);

        if (!empty($this->field)) {
            $datafield = $DB->get_field('course_info_data', 'data', array('courseid' => $courseid, 'fieldid' => $fieldid));
