 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class cohort_field_checkbox extends cohort_profile_field_base {
     ...
     * @param int $cohortid
     */
    public function cohort_field_checkbox($fieldid=0, $cohortid=0) {
        global $DB;
        // First call parent constructor.
        $this->cohort_profile_field_base($fieldid, $cohortid);

        if (!empty($this->field)) {
            $datafield = $DB->get_field('cohort_info_data', 'data', array('cohortid' => $cohortid, 'fieldid' => $fieldid));
