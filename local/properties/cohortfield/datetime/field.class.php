 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU Public License
 */
class cohort_field_datetime extends cohort_profile_field_base {
    ...
    public function edit_save_data_preprocess($datetime, $datarecord) {     
       ...
       /* 
        * This code is written to fix the date time profile save issue .
        * @auther alok kumar
        * Since 24-July-2016
        */
        if(is_array($datetime)  &&( isset( $datetime['year']) && isset($datetime['month']) && isset($datetime['day'])))
        {           
        $date = new DateTime($datetime['year'].'-'.$datetime['month'].'-'.$datetime['day'].' 00:00:00');
