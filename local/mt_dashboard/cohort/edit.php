...
require($CFG->dirroot.'/local/mt_dashboard/cohort/lib.php');
require($CFG->dirroot.'/local/mt_dashboard/cohort/edit_form.php');
/**
* Include usercohort.php of local/properties
* @author Jorge M.
* @since Sept 4 of 2018
* @paradiso
*/
require($CFG->dirroot.'/local/properties/usercohort.php');
...
require_capability('local/mt_dashboard:managecompanycohort', $context);

if ($returnurl) {
    $returnurl = new moodle_url($returnurl);
} else {
    $returnurl = new moodle_url('/local/mt_dashboard/cohort/index.php', array('contextid'=>$context->id));
}
...
$baseurl = new moodle_url('/local/mt_dashboard/cohort/edit.php', array('contextid' => $context->id, 'id' => $cohort->id));
...
if ($context->contextlevel == CONTEXT_COURSECAT) {
    ...
    navigation_node::override_active_url(new moodle_url('/local/mt_dashboard/cohort/index.php', array('contextid'=>$cohort->contextid)));

} else {
    navigation_node::override_active_url(new moodle_url('/local/mt_dashboard/cohort/index.php', array()));
}

if ($delete and $cohort->id) {
    $PAGE->url->param('delete', 1);
    if ($confirm and confirm_sesskey()) {
        mt_dashboard_cohort_delete_cohort($cohort);
        redirect($returnurl);
    }
    ...
    $yesurl = new moodle_url('/local/mt_dashboard/cohort/edit.php', array('id' => $cohort->id, 'delete' => 1,
...
/**
* load custom fields data and added function cohort_profile_save_data to save data
* @author Jorge M.
* @since Sept 4 of 2018
* @paradiso
*/
cohort_profile_load_data($cohort);
...
if ($editform->is_cancelled()) {
    ...
    $companyname = mt_dashboard_get_company_shortname($companyid);
    ...
    if ($data->id) {
        ...
        mt_dashboard_cohort_update_cohort($data);
        cohort_profile_save_data($data);
    } else {
        $data->descriptionformat = $data->description_editor['format'];
        $data->description = $description = $data->description_editor['text'];
        $data->id = mt_dashboard_cohort_add_cohort($data);
        $editoroptions['context'] = $context = context::instance_by_id($data->contextid);
        $data = file_postupdate_standard_editor($data, 'description', $editoroptions,
                $context, 'cohort', 'description', $data->id);
        if ($description != $data->description) {
            ...
            mt_dashboard_cohort_update_cohort($updatedata);
        }
        cohort_profile_save_data($data);
    }
    ...
    if ($returnurl->get_param('showall') || $returnurl->get_param('contextid') == $data->contextid) {
        ...
        redirect(new moodle_url('/local/mt_dashboard/cohort/index.php', array('contextid' => $data->contextid)));
    }
}

echo $OUTPUT->header();
echo $OUTPUT->heading($strheading);
if (!$id && ($editcontrols = mt_dashboard_cohort_edit_controls($context, $baseurl))) {
