...
class cohort_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        if (!empty($CFG->allowcohortthemes)) {
            $themes = array_merge(array('' => get_string('forceno')), mt_dashboard_cohort_get_list_of_themes());
        ...
        * @paradiso
        */
        cohort_profile_definition($mform, $cohort->id);
    ...
    public function validation($data, $files) {
        ...
        if ($idnumber === '') {
            ...
            if ($current->idnumber !== $idnumber) {
                if ($DB->record_exists_sql("SELECT c.idnumber FROM {cohort} c left join {company_cohorts} cc on c.id=cc.cohortid WHERE c.idnumber = '$idnumber' and cc.companyid = $companyid")) {
    ...
    protected function get_category_options($currentcontextid) {
       ...
       // require_once($CFG->libdir. '/coursecatlib.php');
        $displaylist = \core_course_category::make_categories_list('local/mt_dashboard:companycohort_view');//coursecat::make_categories_list('local/mt_dashboard:companycohort_view');
        $options = array();
        $syscontext = context_system::instance();
        if (has_capability('local/mt_dashboard:companycohort_view', $syscontext)) {
            $options[$syscontext->id] = $syscontext->get_context_name();
        }
        
        $companyid = getcompanyid();
        $companyCategory = mt_dashboard_get_company_category($companyid);
        $cmpcontext = context_coursecat::instance($companyCategory);
        foreach ($displaylist as $cid=>$name) {
            $context = context_coursecat::instance($cid);
            if($cmpcontext->id == $context->id && $currentcontextid == 1){
