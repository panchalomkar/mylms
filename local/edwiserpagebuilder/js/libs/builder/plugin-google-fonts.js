GoogleFontsManager = {
	...
	updateFontList: function () {
		let googleFontsLink = $("#google-fonts-link", Vvveb.Builder.frameBody);


		var fontUrl = googleFontsLink.attr('href');

		// Extract font families from the URL parameters
		if(fontUrl != undefined){
			var fontFamilyParam = fontUrl.split("?")[1];
			var fontFamilies = fontFamilyParam.split("&family=");
			fontFamilies.shift();
			fontFamilies = fontFamilies.map(function(family) {
			return family.replace(/\+/g, " ");
			});
		    this.activeFonts = this.activeFonts.concat(fontFamilies);
			this.activeFonts = [...new Set(this.activeFonts)];
		}
		...
		if (googleFontsLink.length < 1) {
			...
			Vvveb.Builder.frameBody.append(googleFontsLink);
		}
