...
Vvveb.Builder = {
	...
	init: function(url, callback) {
		...
		self._loadRowCreator();
		// self._adjustIframeHeight();

		self._addExtraEvents();

	},

	_addExtraEvents: function() {
		$(".components-list label.header").on("click", function(){
			var inode = this.nextElementSibling;
			if($(inode).is(":checked")){
				// Rotating it to 0 deg as we already are having it rotated at 180deg
				$(this).find('.header-arrow').addClass("rotate0");
			} else {
				// Rotating it to 0 deg as we already are having it rotated at 180deg
				$(this).find('.header-arrow').removeClass("rotate0");
			}
		});
	},
	...
	_adjustIframeHeight : function () {
		delay(() => {
			...
			var newHeight = frmdiv.contentWindow.document.body.scrollHeight;
			frmdiv.style.height = 0;
			frmdiv.style.height = newHeight + 'px';
		}, 1000);
	},
	_loadRowCreator : function() {
		var _self = this;
		$(".clickable-layer").on("click", function(){
			$(".rowcontainer.rowselection").show();
			$(".rowcontainer.rowselectorbtn").hide();
		});

		$(".rowselection .drow").on("click", function () {
			var rowhtml = $(tmpl("vvveb-section-rowcreator", {rows: $(this).data("rows")}));
			rowhtml = $(rowhtml.get(0)).html();
			$(_self.frameBody).append(rowhtml);
			// _self.loadSections();
			Vvveb.SectionList.loadSections();
			// _self._adjustIframeHeight();

			$(".rowcontainer.rowselection").hide();
			$(".rowcontainer.rowselectorbtn").show();
		});

		$(".rowselection .close").on("click", function(){
			$(".rowcontainer.rowselection").hide();
			$(".rowcontainer.rowselectorbtn").show();
		});
	},
			...
			for (group in Vvveb.ComponentsGroup)
			{
				...
				for (i in components)
				{
					...
					if (component)
					{
						...
						if (component.image) {
							// var image = ((component.image.indexOf('/') == -1) ? Vvveb.imgBaseUrl:'') + component.image;

							item.find("img").attr("src", Vvveb.imgBaseUrl + component.image);
							// item.css({
	...
	_loadIframe : function(url) {
		...
		return this.documentFrame.on("load", function()
        {
				...
				Vvveb.Builder._adjustIframeHeight();
				Vvveb.SectionList.loadSections();
		...
		self.frameHtml.on("click", function(event) {
			...
			if (Vvveb.Builder.isPreview == false){
				if (event.target) {
					...
					// if ($(".component-properties-tab").is(":visible"))//if properites tab is enabled/visible
					// 	$('.component-properties-tab a').show().tab('show');
					var tab = new bootstrap.Tab(document.querySelector('#properties-tab'));
					tab.show();
	...
	_initBox: function() {
		...
		$("#delete-btn").on("click", function(event) {
			...
			// Restructure Sections according to the content modification
			Vvveb.SectionList.loadSections();
		...
		function addSectionComponent(html, after = true) {
			...
			if ($(addSectionElement)[0].classList.contains('has-dummy')) {
				$(addSectionElement).removeClass('has-dummy');
				$(addSectionElement).find('.add-content').remove();
			}
...
Vvveb.CodeEditor = {
	...
	setValue: function(value) {
		...
		{
			// $("#vvveb-code-editor textarea").val(Vvveb.Builder.getHtml());
			$("#vvveb-code-editor textarea").val(Vvveb.Builder.frameBody.html().trim());
		}
	},
...
Vvveb.Gui = {
	...
	init: function() {
		...
		var iframe = $('#iframe1.epb-editor-iframe');
		// Add a load event listener to the iframe
		iframe.on('load', function() {
			// Access the iframe's content window
			var iframeContentWindow = iframe[0].contentWindow;
			// Event listener for Ctrl+Z (Undo)
			$([window,iframeContentWindow.document]).on('keydown', function(event) {
			...
			$([window,iframeContentWindow.document]).on('keydown', function(event) {
				// Check if Ctrl key is pressed and the 'R' key (key code 82) is pressed
				if (event.altKey && event.keyCode === 82) {
				  // Prevent the default Ctrl+R behavior (page refresh)
				  event.preventDefault();
				  Vvveb.Gui.resetpage();
				}
			  });
	...
	search : function () {
		...
		let panel = $("div > ul", this.parentNode.parentNode.parentNode)
	...
	expand : function (e) {
		...
		$('label.header .header-arrow.rotate0', this.parentNode.parentNode.parentNode).removeClass("rotate0");
	},

	collapse : function (e) {
		$('input.header_check[type="checkbox"]', this.parentNode.parentNode.parentNode).prop("checked", false);
		$('label.header .header-arrow', this.parentNode.parentNode.parentNode).addClass("rotate0");
	},
	...
	toggleLeftColumn: function () {
		...
		$("#content-panel").toggleClass("showleftpanel");
	},
	...
	// Reset the page to last saved milestone.
	resetpage : function () {
		function myFunction() {
			let text = "All unsaved changes will be lost.\nAre you sure you want to reset the blocks?";
		...
		if (doc) {
			...
			for (let j = 0; j < style.cssRules.length; j++) {
				media = (typeof style.cssRules[j].media === "undefined") ?
					...
					? "tablet" : (style.cssRules[j].media[0] === "screen and (max-width: 599px)")
					? "mobile" : "desktop";
				...
				if (media) {
					if (this.styles[media] == undefined) {
						this.styles[media] = {};
					}
					if (selector) {
...
Vvveb.FileManager = {
	...
	deletePage: function(element, e) {
		// console.log("FileManager: deletePage")
	...
	renamePage: function(element, e, duplicate = false) {
		// console.log("FileManager: renamePage")
		// let page = element[0].dataset;
	...
	cleanUnusedFonts: function (){
		for (i in this.activeFonts) {
			...
			if (elementFont.element) {
				...
				if (Vvveb.StyleManager.getStyle($(elementFont.element),'font-family') != elementFont.fontFamily) {
